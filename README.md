...
- Robot-side on-policy checkpoints: `smolVLA_task_box_750`, `smolVLA_task_box_1050`, `smolVLA_task_box_1050_toponly`
- Additional D-condition analysis: `smolVLA_task_box_795` was used for cross-dump (`D@A`)
- Robot-side on-policy evaluation data: `12` episodes per checkpoint, `36` episodes total (A/B/C rollout set)
- Object placement layouts: two versions (`6` episodes + `6` episodes) per checkpoint

Evaluation dataset naming convention:
- `eval_task_box_<N>_B`: on-policy rollout frames generated by model **B** (contains `data/meta/videos/frames`).
- `eval_task_box_<N>_C`: attention dumps from model **C** on top of `..._B` frames (`C@B`, usually dump artifacts only).
- Same rule is used for other cross-dump folders (`X@Y`: dump model `X`, frame source `Y`).
- Example: `eval_task_box_1050_B = B@B`, `eval_task_box_1050_C = C@B`.
...
FRAME_TAG=eval_task_box_1050_B
...
lerobot-record \
  ...
  --dataset.root=/home/etri01/model/${FRAME_TAG} \
  --dataset.push_to_hub=false \
...
# --episodes 0-35
# For cross-dump (example: C@B), set:
# FRAME_TAG=eval_task_box_1050_B
# DUMP_TAG=eval_task_box_1050_C
# RUN_TAG=smolVLA_task_box_1050_toponly
```
